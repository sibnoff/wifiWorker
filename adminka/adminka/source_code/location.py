...
# from clientAndHotspot import Hotspot
...
class Location:
    ...
    def get_current_loc(self, timeout):
        ...
        for i in range(timeout):
            Location.connect()
            packet = Location.get_current()
            if packet.mode >= 2:
                data['lat'] = packet.position()[0]
                data['lon'] = packet.position()[1]
                break
            time.sleep(1)
...
# lc = Location()
# for i in range(600):
#     bssid = 'test-bssid-{}'.format(random.randint(0, 100))
#     essid = 'test-essid-{}'.format(random.randint(0, 100))
#     gps = lc.get_current_loc(10)
#     print(bssid, essid, gps)
#     hc = Hotspot(bssid, essid, gps['lat'], gps['lon'])
#     hc.insert_info()
#     time.sleep(10)
